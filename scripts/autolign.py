...
import q_learn_autolign as qlearn
...
def simLoop(i):
    ...
    guess = qlearn.qLearning(del_cmd, state, dt, path, i)  # calc RL
...
if __name__ == '__main__':			# main function
    ...
    if mode == 'exp':               # Mode == exp
        ...
        print ("X1's simulated initial misalignment is: "
               + str(misalign) + ' radians\n')
        state_0 = {'E': path['E'] + 0,  # error
                   'N': path['N'] + 0,  # error
                   'psi': path['psi'],
                   'Ux': path['v'],     # error
                   'Uy': 0, 'r': 0}		# initialize state
    ...
    for i in range(n):
        state, guess = simLoop(i) 	# run simulation loop
        E = np.r_[E, state['E']]
        N = np.r_[N, state['N']]
    ...
    for j in range(n):
        ...
        e = ctrl.calcLateralError(path, state)
        v = state['Ux']
        print ("j: ", j, "E:", round(state['E'], 2),
               " N:", round(state['N'], 2), " Psi:", round(state['psi'], 2),
               " Ux:", round(state['Ux'], 2), " Uy:", round(state['Uy'], 2),
               " r:", round(state['r'], 2))
        del_cmd = ctrl.lookAheadCtrl(path, state)	        # calc steer cmds
        del_cmd -= guess					# apply misalignment and guess
        Fxr = ctrl.PI_Ctrl(path, state)		# calc longitudinal cmd (rear axle)
